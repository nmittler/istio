...
func TestServiceSameVersionShouldBeIgnored(t *testing.T) {
	h, l := newHandler()

	serviceName := name("ns", "svc1")
	createTime := time.Now()
...
func TestEndpointsWithSameVersionShouldBeIgnored(t *testing.T) {
	h, l := newHandler()
	...
	t.Run("UpdateSameVersionNoChange", func(t *testing.T) {
		h.Handle(resource.Event{
			Kind:  resource.Added,
			Entry: endpointsEntry(serviceName, createTime, "v1", pod1IP),
		})
