...
func TestQueue(t *testing.T) {
	q := queue.NewQueue(1 * time.Microsecond)
	...
	q.Push(queue.NewTask(add, 1, model.EventAdd))
	q.Push(queue.NewTask(add, 1, model.EventAdd))
	q.Push(queue.NewTask(func(obj interface{}, event model.Event) error {
		out = out + obj.(int)
		if out != 3 {
...
func TestChainedHandler(t *testing.T) {
	...
	out := 0
	f := func(i int) queue.Handler {
		return func(obj interface{}, event model.Event) error {
			out = out + i
			return nil
		}
	}
	handler := queue.NewChainHandler(f(1), f(2))
	go q.Run(stop)

	q.Push(queue.NewTask(handler.Apply, 0, model.EventAdd))
	q.Push(queue.NewTask(func(obj interface{}, event model.Event) error {
		if out != 3 {
